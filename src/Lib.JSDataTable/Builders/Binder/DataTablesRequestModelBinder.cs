...
namespace Lib.DataTablesNet.Builders.Binder
{
    class DataTablesRequestModelBinder : IModelBinder
    {
        ...
        private void BindModel(ModelBindingContext bindingContext)
        {
            ...
            TryParse(valueResultProvider, out draw);
            TryParse(valueProvider.GetValue("start"), out start);
            TryParse(valueProvider.GetValue("length"), out length);
        ...
        {
            string searchValue;
            if (TryParse(valueProvider.GetValue("search[value]"), out searchValue) &&
                !string.IsNullOrEmpty(searchValue))
            {
                bool regex = false;
                TryParse(valueProvider.GetValue("search[regex]"), out regex);
        ...
        private IEnumerable<DataTableColumn> TryGetColumns(IValueProvider valueProvider)
        {
            ...
            do
            {
                if (valueProvider.GetValue($"columns[{index}][data]").FirstValue != null)
                {
                    ...
                    TryParse(valueProvider.GetValue($"columns[{index}][data]"), out data);
                    TryParse(valueProvider.GetValue($"columns[{index}][name]"), out name);
                    TryParse(valueProvider.GetValue($"columns[{index}][searchable]"), out searchable);
                    TryParse(valueProvider.GetValue($"columns[{index}][orderable]"), out orderable);
                    TryParse(valueProvider.GetValue($"columns[{index}][search][value]"), out searchValue);
                    TryParse(valueProvider.GetValue($"columns[{index}][search][regex]"), out searchRegEx);
        ...
        private IEnumerable<Order> TryGetOrders(IValueProvider valueProvider)
        {
            ...
            do
            {
                if (valueProvider.GetValue($"order[{index}][column]").FirstValue != null)
                {
                    ...
                    TryParse(valueProvider.GetValue($"order[{index}][column]"), out column);
                    TryParse(valueProvider.GetValue($"order[{index}][dir]"), out dir);
